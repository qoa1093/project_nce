...
public class ListTest1 {
	...
	public static void main(String[] args) {
		...
		//제네릭 클래스는 타입을 클래스로 해야한다. 일반 변수 x
		ArrayList<String> list = new ArrayList<String>(); 
		//문자열로 이루어진 리스트(객체생성시 <>안에 넣는 클래스의 타입으로 결정됨)
		// add() : 리스트에 추가
		for(int i =0; i<10;i++) {
			list.add("학원 출근");
		}
		...
		//indexOf(값) : 값이 리스트에 있는지 없는지 확인하여 있으면 번지를 없으면 -1을 반환. 해당 클래스의 equals()를 호출하여 같은지를 확인
