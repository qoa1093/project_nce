...
public class Accountbook {
	...
	//set을 하면 임의로 정해진 숫자 바꾸는거라서 밖에서 바꿀수 있음. arr[]는 get도 안됨 참조변수는 주소를 저장하므로 주소를 알면 밖에서 남이 수정할 수 있음..
	public int getCount() {
		return count;
	}
	public int getTotal() {
	...
	/* 기능 : 내역 전체를 자세히 확인하는 메소드(을 리턴타입없이 출력으로만?? 그러면 item arr[]을 하나씩 꺼내서 바로바로 출력하는 반복문 넣은 메소드?)
	 * 매개변수 : 없음
	 * 리턴타입 : 없음 => void
