...
public class PageMaker {
	...
	public void calcData() {
		...
		//네이버 예시 : endPage = (int)(Math.ceil(3/(double) 10) *) ceil은 소숫점 첫번째자리에서 올림함 => 1, endpage 10이 됨
		startPage = (endPage - displayPageNum)+1;
		/* 총 콘텐츠 갯수를 이용하여 실제 마지막 페이지 번호를 계산 
		 * 총 콘텐츠 갯수가 61개이고 한 페이지에 컨텐츠 수가 20개
		 * */
		int tempEndPage = (int)(Math.ceil(totalCount/(double)criteria.getPerPageNum()));
		//tempEncPage = (int) (Math.ceil(61/20.0))3.05 => 4가 됨. 10>4이므로 마지막 페이지 4로 바꿈
