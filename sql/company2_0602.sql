...
월급 - 월급은 직급별 기본급에 직급별 호봉에 따른 추가금 
...
/*
직급 
- 직급명, 사원명, 입사년차, 사원번호(기본키), 기본급, 호봉
월급
- 월급번호(기본), 월급, 직급명(외래), 사원명(외래), 사원번호(외래), 입사년차(외래)
...
create table if not exists salary(
    ...
    em_base_salart int not null default 0,
...
insert into salary 
	values('사원',200,10),('대리',220,15),('과장',240,20),('부장',280,25),('전무',300,30),('이사',350,35);

insert into employee
	values(1, '홍길동', '사원', 2, 220);

-- 호봉이나 직급이 변경되면 월급이 변경되는 트리거 또는 프로시저
